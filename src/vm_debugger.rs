...
use crate::vm_display::{Formatter, InstructionDisplay, Token};
use crate::vm_shell::{RunResult, VMShell, CPU};
    ...
    > Debugger<'a, RegKey, RegVal, Instr>
{
    ...
    fn run_impl<B: Backend>(&mut self, mut terminal: Terminal<B>) -> io::Result<()> {
        ...
        loop {
            ...
            } else if self.run_state != RunState::Walk && self.run_state != RunState::Run
                || event::poll(timeout)?
            {
    ...
    fn ui<B: Backend>(&mut self, f: &mut Frame) {
        ...
        let rightside = Layout::default()
            ...
            .constraints([Constraint::Min(10), Constraint::Length(sp_height)].as_ref())
            .split(root[1]);
        self.render_code::<B>(f, root[0]);
        self.render_registers::<B>(f, rightside[0]);
    ...
    fn render_pc_input<B: Backend>(&self, f: &mut Frame, rect: Rect) {
        ...
        f.set_cursor_position((
            rect.x + ((self.pc_input.visual_cursor()).max(scroll) - scroll) as u16 + 1,
            rect.y + 1,
        ));
    }
    ...
    fn token_list(&self, iter: &mut dyn Iterator<Item = Vec<Token<RegVal>>>) -> Vec<Line> {
        ...
        rows.iter().for_each(|row| {
            row.iter()
                .enumerate()
                .for_each(|(idx, t)| colwidths[idx] = colwidths[idx].max(t.width()));
        });

        rows.iter()
            .map(|row| {
                Line::from_iter(row.iter().enumerate().flat_map(|(idx, t)| {
                    let w = t.width();
    ...
    fn render_code<B: Backend>(&self, f: &mut Frame, rect: Rect) {
        let mut itr = self
            .shell
            .vm
            .program
    ...
    fn render_registers<B: Backend>(&self, f: &mut Frame, rect: Rect) {
        let mut itr = self
            .shell
            .vm
            .registers
            ...
            .map(|(k, v)| vec![Token::Register(k.clone().into()), Token::Integer(*v)]);
        let rows: Vec<ListItem> = self
            .token_list(&mut itr)
            .into_iter()
            .map(ListItem::new)
        ...
        let list = List::new(rows).block(Block::default().borders(Borders::ALL).title("Registers"));
